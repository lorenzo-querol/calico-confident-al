...
def main(config):
    ...
    (
        ...
        sampling_method="equal_labels" if config["experiment_type"] == "equal_labels" else "random",
    ...
    for i in range(n_iters):
        ...
        elif config["experiment_type"] == "equal_labels":
            """---BASELINE STEP---"""
            init_size += config["query_size"]
            (
                dload_train,
                ...
                train_labeled_inds,
                train_unlabeled_inds,
            ) = datamodule.get_data(start_iter=False, sampling_method="equal_labels")
