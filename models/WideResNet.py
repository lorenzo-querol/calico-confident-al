        ...
        self.block = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("2_activation", nn.LeakyReLU(0.2, inplace=True)),
...
class Block(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, stride: int, depth: int, dropout: float):
        ...
        self.block = nn.Sequential(
            DownsampleUnit(in_channels, out_channels, stride, dropout), *(BasicUnit(out_channels, dropout) for _ in range(depth))
        )
        ...
        self.f = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("5_activation", nn.LeakyReLU(0.2, inplace=True)),
                    ("6_pooling", nn.AvgPool2d(kernel_size=7)),
