...
def init_random(datamodule: MedMNISTDataModule):
    global conditionals
...
def category_mean(datamodule: MedMNISTDataModule):
    img_shape = datamodule.img_shape
    n_classes = datamodule.n_classes
    ...
    im_test, targ_test = [], []
    for im, targ in datamodule.train_dataloader():
        im_test.append(im)
...
def sample_q(f, datamodule, replay_buffer, args, device):
    ...
    for _ in range(args.n_steps):
        ...
        p = 1.0 * f.f.layer_one_out.grad.detach() if in_steps > 0 else None
...
def fit(f: nn.Module, datamodule: MedMNISTDataModule, replay_buffer, args: argparse.Namespace, writer: SummaryWriter, log_dir: str, al_iter: int):
    ...
    - writer: `SummaryWriter`
        - The tensorboard writer to log metrics.
    - log_dir: `str`
        - The directory to save logs.
    - al_iter: `int`
        - The active learning iteration number.
    """
    ...
    progress_bar = tqdm(range(args.n_epochs), desc="Training", total=args.n_epochs, position=0, leave=True)
    ...
    for epoch in progress_bar:
        ...
        if val_loss < best_val_loss:
            t.save(f.state_dict(), f"{ckpt_dir}/labels_{len(datamodule.labeled_indices)}_best.ckpt")
    ...
    t.save(f.state_dict(), f"{ckpt_dir}/labels_{len(datamodule.labeled_indices)}_last.ckpt")
